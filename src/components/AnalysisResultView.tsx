...
import { 
  ...
  Image as ImageIcon, Network, Sparkles, HelpCircle, ShieldCheck
} from 'lucide-react';
...
export const AnalysisResultView: React.FC<AnalysisResultViewProps> = ({ 
  ...
  // Smooth animated risk-score counter from 0 to finalScore
  useEffect(() => {
    ...
    const duration = 1200; // 1.2 seconds smooth animated count
    ...
    const tick = (now: number) => {
      ...
      // Smooth ease-out cubic progression
  ...
  }, [result.riskScore, result.isInsufficientEvidence]);
...
Recommendation: ${result.recommendedActions[0] || 'Verify before paying'}`;
  ...
  return (
    <div className="glass-panel" style={{
      ...
      {isHighRisk && (
        <div className="banner-high-risk-pulse" style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          ...
          {/* Circular Score Visual with Smooth Counter & Shield Animation */}
          <div 
            className={isHighRisk ? 'shield-high-risk' : result.riskLevel === 'LOW' ? 'shield-low-risk' : ''}
            style={{
              position: 'relative',
              width: '84px',
              height: '84px',
              ...
              background: isInsufficient 
                ? '#334155' 
                : `conic-gradient(${riskStyle.text} ${displayScore * 3.6}deg, #1e293b 0deg)`,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
          ...
          >
            <div style={{
              ...
              <span className="score-number-counter" style={{ fontSize: isInsufficient ? '1rem' : '1.5rem', fontWeight: 800, color: riskStyle.text, lineHeight: 1 }}>
                {isInsufficient ? 'N/A' : displayScore}
              </span>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.35rem', flexWrap: 'wrap' }}>
              <span style={{
                ...
                {isHighRisk && <AlertTriangle size={13} className="shield-warning-icon" />}
                {result.riskLevel === 'LOW' && <CheckCircle2 size={13} className="shield-check-icon" />}
                {riskStyle.label}
              </span>
      ...
      {/* VERIFICATION STATUS (Objective Ground-Truth Indicators) */}
      <div style={{
        padding: '1.25rem',
        borderRadius: '0.75rem',
        backgroundColor: '#0b1120',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '0.85rem' }}>
          ...
          <div style={{
            ...
            <div style={{
              fontSize: '0.86rem',
              fontWeight: 700,
              color: result.verificationStatus?.businessVerification && result.verificationStatus.businessVerification !== 'Not provided' ? '#34d399' : '#94a3b8'
            }}>
              {result.verificationStatus?.businessVerification || 'Not provided'}
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{
              fontSize: '0.86rem',
              fontWeight: 700,
              color: result.verificationStatus?.paymentVerification && result.verificationStatus.paymentVerification !== 'Not provided' ? '#34d399' : '#94a3b8'
            }}>
              {result.verificationStatus?.paymentVerification || 'Not provided'}
            </div>
          </div>

          {/* Community Match Status */}
          <div style={{
            padding: '0.85rem 1rem',
            borderRadius: '0.5rem',
            backgroundColor: '#151e36',
            ...
            <div style={{
              fontSize: '0.86rem',
              fontWeight: 700,
              color: result.communityMatch?.hasMatch ? '#f87171' : '#94a3b8'
            }}>
              {result.communityMatch?.hasMatch
                ? `Match found (${result.communityMatch.corroboratingReportsCount} reports in demo dataset)`
                : 'No matching record found'}
            </div>
          </div>
        </div>

        <div style={{ marginTop: '0.75rem', fontSize: '0.74rem', color: '#64748b', fontStyle: 'italic' }}>
          Absence of verified registration or community reports does not prove legitimacy. Always verify seller credentials independently.
        </div>
      </div>
      ...
      {/* Official Assessment Disclaimer Footer */}
      <div style={{
        marginTop: '1.5rem',
        ...
        <strong style={{ color: '#94a3b8' }}>Disclaimer:</strong> {result.assessmentDisclaimer} ScamShield provides risk assessment and safety guidance for fraud awareness; it does not determine legal or criminal liability.
      </div>
